...
- 序列化与模板
  ...
  - RedisCacheManager 默认 TTL 为 30 分钟；针对 sectors、sectorDetail、userInfo 缓存区分别设置更短 TTL（5/10 分钟），降低陈旧数据影响。
- 键设计策略
...
## 结论
本项目通过注解缓存、Token 黑名单、用户收藏缓存与滑动窗口限流，构建了兼顾性能与安全的缓存与限流体系。结合 MyBatis-Plus 分页与可调连接池，可在高并发场景下保持稳定与低延迟。建议持续监控缓存命中率与限流触发情况，动态优化 TTL 与阈值，配合压测与基准测试，持续提升系统整体性能。
