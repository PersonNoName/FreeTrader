...
DTO["AuthResponse/LoginRequest/RefreshTokenRequest"]
end
subgraph "前端"
...
- JwtUtils：提供令牌生成、解析、验证、类型判断与过期时间查询等能力，基于 HMAC 密钥与过期时间配置。
- SecurityConstants：集中定义令牌前缀、请求头名、令牌类型声明键及类型枚举。
...
### JwtUtils 工具类
- 密钥与过期时间：从配置读取 secret、access-token-expiration、refresh-token-expiration，最小密钥长度校验与填充。
- 令牌生成：
...
图表来源
- [JwtUtils.java](file://backend/src/main/java/com/freetrader/security/JwtUtils.java#L25-L194)
...
- JwtUtils 依赖 SecurityConstants、配置项（secret、过期时间）。
...
- 过期时间配置：Access Token 过期时间较短，Refresh Token 较长，降低泄露风险。
- 密钥安全：生产环境必须使用足够强度的密钥并通过环境变量注入，避免硬编码。
...
## 结论
FreeTrader 的 JWT 认证体系通过 JwtAuthFilter、JwtUtils、TokenService、UserService 与 CacheService 的协同，实现了“短效 Access Token + 长效 Refresh Token”的双令牌模型，并结合黑名单机制提升安全性。前端通过 axios 拦截器与 Zustand 状态管理，保证了令牌传递与用户体验的一致性。建议在生产环境严格配置密钥与过期时间，并对关键接口进行限流与监控。
